...
struct DB {
  ...
  int numItems;              // # of stored items, needed to determine
			     ...
			     // # or not
  struct BloomFilter* bf_name;  // Counting Bloom filter for name
  struct BloomFilter* bf_id;    // Counting Bloom filter for id
};
...
CreateCustomerDB(void)
{
  ...
  d->bf_id = bloomfilter_create(BLOOMFILTER_SIZE, NUM_HASH);
  d->bf_name = bloomfilter_create(BLOOMFILTER_SIZE, NUM_HASH); 
  return d;
...
FindCustomerPositionByID(DB_T d, const char *id)
{
  ...
  // bloomfilter
  if (!bloomfilter_check(d->bf_id, id)){
		fprintf(stderr, "No such customer id\n");
		return(-1);
  }
...
FindCustomerPositionByName(DB_T d, const char *name)
{
  ...
  // bloomfilter
  if (!bloomfilter_check(d->bf_name, name)){
		fprintf(stderr, "No such customer name\n");
		return(-1);
  }
...
DestroyCustomerDB(DB_T d)
{
  ...
  if(d -> pArray !=NULL)   //free pArr if exists
    free(d -> pArray);
  bloomfilter_destroy(d->bf_id);
  bloomfilter_destroy(d->bf_name);  
  free(d);
}
		 ...
		 const char *name, const int purchase)
{
  ...
  */
  ...
  // Use this one because of possible false positive results
  // in bloom filter
  ...
  // bloomfilter add information
  bloomfilter_add(d->bf_name, name);  
  bloomfilter_add(d->bf_id, id);
...
UnregisterCustomerByID(DB_T d, const char *id)
{
	...
	if(customerPosition == -1){
		...
		d->numItems--;
    // bloomfilter add information
    bloomfilter_remove(d->bf_id, id);
    bloomfilter_remove(d->bf_name, d->pArray[d->numItems].name);
...
UnregisterCustomerByName(DB_T d, const char *name)
{
    ...
    // bloomfilter add information
    bloomfilter_remove(d->bf_id, d->pArray[d->numItems].id);
    bloomfilter_remove(d->bf_name, name);
